 ...
 * 覆盖：暂停确认、PAUSED 冻结取数、恢复续投、重复置意图幂等、意图切换竞态。
 */
public class DispatcherPauseTest {
    ...
    public void pausesBetweenMessages() throws Exception {
        ...
        awaitTrue(() -> dispatcher.getPhase() == Dispatcher.PausePhase.PAUSED,
                2000, "暂停应在线程确认后到达终态");
        Thread.sleep(200);
    ...
    public void pausedStopsPolling() throws Exception {
        ...
        awaitTrue(() -> dispatcher.getPhase() == Dispatcher.PausePhase.PAUSED, 2000, "暂停应确认");
    ...
    public void resumeRestoresDelivery() throws Exception {
        ...
        awaitTrue(() -> dispatcher.getPhase() == Dispatcher.PausePhase.RUNNING, 2000, "应翻回运行态");
    ...
    public void pauseAndResumeIdempotent() throws Exception {
        ...
        dispatcher.requestPause();
        dispatcher.requestPause();             // 重复置意图无害
        awaitTrue(() -> dispatcher.getPhase() == Dispatcher.PausePhase.PAUSED,
                2000, "暂停应确认");
        int frozen = pollCount.get();
        Thread.sleep(200);
        ...
        dispatcher.requestRun();
        dispatcher.requestRun();               // 重复置意图无害
        awaitTrue(() -> dispatcher.getPhase() == Dispatcher.PausePhase.RUNNING,
                2000, "应翻回运行态");
        dispatcher.stop();
    }
    ...
    public void rollbackRaceRecovers() throws Exception {
        for (int round = 0; round < 20; round++) {
            ...
            awaitTrue(() -> dispatcher.getPhase() == Dispatcher.PausePhase.RUNNING, 2000,
                    "round " + round + " 终态应恢复运行（假死实现在此超时转红）");
